---
name: open-project-tree-selector
description: Open the current project folder in a Codex in-app-browser file tree selector powered by @pierre/trees.
---

# Open Project Tree Selector
...
1. Call the `project_tree_start` MCP tool. When this plugin is installed as a home-local plugin, pass the current workspace root as the `root` argument.
...
- If `project_tree_start` reports that the app is not built, run `npm install && npm run build` from the plugin directory, then call the tool again.
